...
public class MealPlanResource {
    ...
    public ResponseEntity<List<MealPlanDTO>> getAllMealPlans(
        ...
        if ("cartitem-is-null".equals(filter)) {
            log.debug("REST request to get all MealPlans where cartItem is null");
            return new ResponseEntity<>(mealPlanService.findAllWhereCartItemIsNull(), HttpStatus.OK);
        }

        if ("orderitem-is-null".equals(filter)) {
            log.debug("REST request to get all MealPlans where orderItem is null");
            return new ResponseEntity<>(mealPlanService.findAllWhereOrderItemIsNull(), HttpStatus.OK);
        }
