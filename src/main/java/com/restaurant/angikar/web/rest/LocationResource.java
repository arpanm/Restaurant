...
/**
 * REST controller for managing {@link com.restaurant.angikar.domain.Location}.
 */
@RestController
...
public class LocationResource {
    ...
    public ResponseEntity<List<LocationDTO>> getAllLocations(
        ...
        if ("restaurant-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where restaurant is null");
            return new ResponseEntity<>(locationService.findAllWhereRestaurantIsNull(), HttpStatus.OK);
        }

        if ("mealplanitem-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where mealPlanItem is null");
            return new ResponseEntity<>(locationService.findAllWhereMealPlanItemIsNull(), HttpStatus.OK);
        }

        if ("orderitem-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where orderItem is null");
            return new ResponseEntity<>(locationService.findAllWhereOrderItemIsNull(), HttpStatus.OK);
        }

        if ("order-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where order is null");
            return new ResponseEntity<>(locationService.findAllWhereOrderIsNull(), HttpStatus.OK);
        }
