...
public class MealPlanServiceImpl implements MealPlanService {
    ...
    /**
     *  Get all the mealPlans where OrderItem is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<MealPlanDTO> findAllWhereOrderItemIsNull() {
        log.debug("Request to get all mealPlans where OrderItem is null");
        return StreamSupport
            .stream(mealPlanRepository.findAll().spliterator(), false)
            .filter(mealPlan -> mealPlan.getOrderItem() == null)
            .map(mealPlanMapper::toDto)
            .collect(Collectors.toCollection(LinkedList::new));
    }
