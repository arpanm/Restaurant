...
export const Restaurant = () => {
  ...
  const links = useAppSelector(state => state.restaurant.links);
  ...
  return (
    <div>
      <h2 id="restaurant-heading" data-cy="RestaurantHeading">
        <Translate contentKey="restaurantApp.restaurant.home.title">Restaurants</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="restaurantApp.restaurant.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/restaurant/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="restaurantApp.restaurant.home.createLabel">Create new Restaurant</Translate>
          </Link>
        </div>
      </h2>
          ...
          {restaurantList && restaurantList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="restaurantApp.restaurant.id">ID</Translate>{' '}
                  ...
                  <th>
                    <Translate contentKey="restaurantApp.restaurant.location">Location</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {restaurantList.map((restaurant, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>
                      <Translate contentKey={`restaurantApp.RestaurantType.${restaurant.type}`} />
                    </td>
                    <td>{restaurant.location ? <Link to={`/location/${restaurant.location.id}`}>{restaurant.location.id}</Link> : ''}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="restaurantApp.restaurant.home.notFound">No Restaurants found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
