  ...
  return (
    <div>
      <h2 id="otp-heading" data-cy="OtpHeading">
        <Translate contentKey="restaurantApp.otp.home.title">Otps</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="restaurantApp.otp.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/otp/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="restaurantApp.otp.home.createLabel">Create new Otp</Translate>
          </Link>
        </div>
      </h2>
          ...
          {otpList && otpList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="restaurantApp.otp.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  ...
                  <th className="hand" onClick={sort('otp')}>
                    <Translate contentKey="restaurantApp.otp.otp">Otp</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('otp')} />
                  </th>
                  ...
                  <th>
                    <Translate contentKey="restaurantApp.otp.usr">Usr</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {otpList.map((otp, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{otp.expiry ? <TextFormat type="date" value={otp.expiry} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{otp.usr ? <Link to={`/application-user/${otp.usr.id}`}>{otp.usr.id}</Link> : ''}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button
                          onClick={() => (location.href = `/otp/${otp.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="restaurantApp.otp.home.notFound">No Otps found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
